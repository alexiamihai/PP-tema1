...
(define (get-ch-words words ch)
  (filter (lambda (word)  (and (not (null? word))(equal? (car word) ch)))
           words) 
)
...
(define (ast-func suffixes)
  (if (or (null? suffixes) (null? (car suffixes)))
      '()
  (let ((prefix  (car (car suffixes))))
    ; am in prefix primul caracter al sufixelor si apoi noile sufixe fara primul caracter
    (cons (list prefix)
           (map (lambda (suf) (cdr suf)) suffixes)))))
...
(define (cst-func suffixes)
  ...
  (let ((prefix (longest-common-prefix-of-list suffixes)))
    (cons prefix
          ; am eliminat prefixul din sufixele mele (si iau al treilea lucru din rezultatul functiei lcp care reprezinta restul de suf
...
(define (suffixes->st labeling-func suffixes alphabet)
  ...
  (define (build-tree suffixes ch)
    (if (or (null? (get-ch-words suffixes ch)) (null? suffixes))
        '()
        (let* ((grouped-suffixes (get-ch-words suffixes ch))
               ; am aplicat functia de etichetare pe sufixele gasite
               (label (labeling-func grouped-suffixes)))
          (if (null? label)
               '()
              (let (
                    ; creez un subarbore cu sufixele care incep cu (car label)
                    (subtree (suffixes->st labeling-func (cdr label) alphabet)))
                ; lipesc subarborele gasit la eticheta
                (cons (car label) subtree))))))
  ; pt fiecare caracter din alfabet am verificat daca se poate construi o ramura care sa inceapa cu el si daca da am construit-o
  (filter (lambda (subtree) (not (null? subtree))) (map (lambda (ch) (build-tree suffixes ch )) alphabet)))
...
(define (text->st text)
  (lambda (labeling-func)
    (suffixes->st labeling-func
                  ; am adaugat la sufixe $ la final
                 (cons '(#\$) (map (lambda (suf)  (append suf '(#\$))) (get-suffixes text)))
                 ; am scos din textul meu( lacare am adaugat $) toate duplicatele 
                  (sort (remove-duplicates (cons #\$  text) equal?) char<?))
    ))
