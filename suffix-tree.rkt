...
(define (get-ch-branch st ch)
  (cond
    ; daca arborele este vid, nu exista nicio ramura care sa satisfaca conditia
    ((st-empty? st) #f)
    ; daca eticheta primei ramuri incepe cu caracterul ch, returnez acea ramura
    ((char=? ch (car (get-branch-label (first-branch st))))
     (first-branch st))
    ; altfel, caut in restul ramurilor
    (else
